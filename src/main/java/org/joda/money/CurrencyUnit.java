...
public final class CurrencyUnit implements Comparable<CurrencyUnit>, Serializable {
     ...
     */
    private static final String CODE = new String("[A-Z][A-Z][A-Z]");
    /**
     * Map of registered currencies by text code.
     */
    private static final Map<String, CurrencyUnit> currenciesByCode = new HashMap<>();
    /**
     * Map of registered currencies by numeric code.
     */
    private static final Map<Integer, CurrencyUnit> currenciesByNumericCode = new HashMap<Integer, CurrencyUnit>();
    /**
     * Map of registered currencies by country.
     */
    private static final Map<String, CurrencyUnit> currenciesByCountry = new HashMap<String, CurrencyUnit>();
    ...
    public static synchronized CurrencyUnit registerCurrency(
        ...
        if (decimalPlaces < -1 || decimalPlaces > 30) {
            throw new IllegalArgumentException("Invalid number of decimal places :  ["+decimalPlaces+"],["+currencyCode+"]");
        }
     ...
     */
    @GwtIncompatible
    public static CurrencyUnit of(CurrencyData currency) {
    ...
    public String getSymbol(Locale locale) {
        MoneyUtils.checkNotNull(locale, "Locale must not be null");
        try {
            return CurrencyList.get().lookup(code).getCurrencySymbol();
        } catch (IllegalArgumentException ex) {
            return code;
        }
    }
